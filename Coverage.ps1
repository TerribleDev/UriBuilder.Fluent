...
.\tools\OpenCover.4.6.519\tools\OpenCover.Console.exe -target:"C:\Program Files\dotnet\dotnet.exe" -targetargs:" test "" .\src\UriBuilder.Fluent.UnitTests\UriBuilder.Fluent.UnitTests.csproj"" --no-build -f net461" -register:user -filter:"+[UriBuilder*]* -[*Tests]*" -returntargetcode -output:opencover_results.xml
